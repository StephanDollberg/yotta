...
char* serve_200(char* buf, std::size_t content_length, time_t* last_modified,
                std::experimental::string_view content_type) {
    ...
    buf =
        std::copy(content_length_buf, content_length_buf + content_length_buf_size, buf);
    buf += std::sprintf(buf, "%lu", content_length);
    buf = std::copy(content_type_buf, content_type_buf + content_type_buf_size, buf);
...
char* serve_206(char* buf, std::size_t content_length, time_t* last_modified,
                std::size_t range_start, std::size_t range_end, std::size_t file_size) {
...
std::size_t clean_path(const char* path, std::size_t length,
                       char* const normalized_path) {
    ...
    while (r < length) {
        if (path[r] == '/') {
            ...
            if ((rooted && (std::size_t(output - normalized_path) != 1)) ||
                (!rooted && (output - normalized_path != 0))) {
                *output++ = '/';
            }
